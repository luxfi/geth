...
func (a *API) aggregateSignatures(ctx context.Context, unsignedMessage *warp.UnsignedMessage, quorumNum uint64, subnetIDStr string) (hexutil.Bytes, error) {
	...
	canonicalValidatorSet, err := warp.GetCanonicalValidatorSetFromSubnetID(ctx, state, pChainHeight, subnetID)
	if err != nil {
		return nil, fmt.Errorf("failed to get validator set: %w", err)
	}
	if len(canonicalValidatorSet.Validators) == 0 {
	...
	log.Debug("Fetching signature",
		...
		"height", pChainHeight,
		"numValidators", len(canonicalValidatorSet.Validators),
		"totalWeight", canonicalValidatorSet.TotalWeight,
	)

	agg := aggregator.New(aggregator.NewSignatureGetter(a.client), canonicalValidatorSet.Validators, canonicalValidatorSet.TotalWeight)
