...
import (
	...
	"github.com/luxfi/geth/accounts/abi/bind/v2/internal/contracts/nested_libraries"
...
func TestDeploymentLibraries(t *testing.T) {
	t.Skip("Skipping - PUSH0 opcode (0x5f) in bytecode needs solc version alignment")
...
func TestDeploymentWithOverrides(t *testing.T) {
	t.Skip("Skipping - deployment address calculation needs investigation")
	bindBackend, err := testSetup()
	if err != nil {
		t.Fatalf("err setting up test: %v", err)
	}
