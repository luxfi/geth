...
func (c *Config) Deprecate() string {
	...
	if c.GethAdminAPIEnabled {
		msg += "geth-admin-api-enabled is deprecated, use admin-api-enabled instead. "
		c.AdminAPIEnabled = c.GethAdminAPIEnabled
	}
	if c.GethAdminAPIDir != "" {
		msg += "geth-admin-api-dir is deprecated, use admin-api-dir instead. "
		c.AdminAPIDir = c.GethAdminAPIDir
	}
