...
var (
	...
	apricotRulesPhase1 = params.Rules{LuxRules: params.LuxRules{IsApricotPhase1: true}}
	apricotRulesPhase2 = params.Rules{LuxRules: params.LuxRules{IsApricotPhase1: true, IsApricotPhase2: true}}
	apricotRulesPhase3 = params.Rules{LuxRules: params.LuxRules{IsApricotPhase1: true, IsApricotPhase2: true, IsApricotPhase3: true}}
	apricotRulesPhase4 = params.Rules{LuxRules: params.LuxRules{IsApricotPhase1: true, IsApricotPhase2: true, IsApricotPhase3: true, IsApricotPhase4: true}}
	apricotRulesPhase5 = params.Rules{LuxRules: params.LuxRules{IsApricotPhase1: true, IsApricotPhase2: true, IsApricotPhase3: true, IsApricotPhase4: true, IsApricotPhase5: true}}
	apricotRulesPhase6 = params.Rules{LuxRules: params.LuxRules{IsApricotPhase1: true, IsApricotPhase2: true, IsApricotPhase3: true, IsApricotPhase4: true, IsApricotPhase5: true, IsApricotPhasePre6: true, IsApricotPhase6: true, IsApricotPhasePost6: true}}
	banffRules         = params.Rules{LuxRules: params.LuxRules{IsApricotPhase1: true, IsApricotPhase2: true, IsApricotPhase3: true, IsApricotPhase4: true, IsApricotPhase5: true, IsApricotPhasePre6: true, IsApricotPhase6: true, IsApricotPhasePost6: true, IsBanff: true}}
	// cortinaRules    = params.Rules{LuxRules: params.LuxRules{IsApricotPhase1: true, IsApricotPhase2: true, IsApricotPhase3: true, IsApricotPhase4: true, IsApricotPhase5: true, IsApricotPhasePre6: true, IsApricotPhase6: true, IsApricotPhasePost6: true, IsBanff: true, IsCortina: true}}
)
...
// BuildGenesisTest returns the genesis bytes for Geth VM to be used in testing
...
func GenesisVMWithUTXOs(t *testing.T, finishBootstrapping bool, genesisJSON string, configJSON string, upgradeJSON string, utxos map[ids.ShortID]uint64) (chan commonEng.Message, *VM, database.Database, *luxatomic.Memory, *enginetest.Sender) {
	...
	for addr, luxAmount := range utxos {
		...
		if _, err := addUTXO(sharedMemory, vm.ctx, txID, 0, vm.ctx.LUXAssetID, luxAmount, addr); err != nil {
...
// resetMetrics resets the vm node metrics, and allows
...
func TestIssueAtomicTxs(t *testing.T) {
	...
	exportTx, err := vm.newExportTx(vm.ctx.LUXAssetID, importAmount-(2*ap0.AtomicTxFee), vm.ctx.XChainID, testShortIDAddrs[0], initialBaseFee, []*secp256k1.PrivateKey{testKeys[0]})
	if err != nil {
		t.Fatal(err)
	}
...
func TestReissueAtomicTxHigherGasPrice(t *testing.T) {
	...
	for name, issueTxs := range map[string]func(t *testing.T, vm *VM, sharedMemory *luxatomic.Memory) (issued []*atomic.Tx, discarded []*atomic.Tx){
		"single UTXO override": func(t *testing.T, vm *VM, sharedMemory *luxatomic.Memory) (issued []*atomic.Tx, evicted []*atomic.Tx) {
			...
			tx1, err := atomic.NewImportTx(vm.ctx, vm.currentRules(), vm.clock.Unix(), vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*lux.UTXO{utxo})
			if err != nil {
				t.Fatal(err)
			}
			tx2, err := atomic.NewImportTx(vm.ctx, vm.currentRules(), vm.clock.Unix(), vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(common.Big2, initialBaseFee), kc, []*lux.UTXO{utxo})
			if err != nil {
				t.Fatal(err)
			}
		...
		"one of two UTXOs overrides": func(t *testing.T, vm *VM, sharedMemory *luxatomic.Memory) (issued []*atomic.Tx, evicted []*atomic.Tx) {
			...
			tx1, err := atomic.NewImportTx(vm.ctx, vm.currentRules(), vm.clock.Unix(), vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*lux.UTXO{utxo1, utxo2})
			if err != nil {
				t.Fatal(err)
			}
			tx2, err := atomic.NewImportTx(vm.ctx, vm.currentRules(), vm.clock.Unix(), vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(common.Big2, initialBaseFee), kc, []*lux.UTXO{utxo1})
			if err != nil {
				t.Fatal(err)
			}
		...
		"hola": func(t *testing.T, vm *VM, sharedMemory *luxatomic.Memory) (issued []*atomic.Tx, evicted []*atomic.Tx) {
			...
			importTx1, err := atomic.NewImportTx(vm.ctx, vm.currentRules(), vm.clock.Unix(), vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*lux.UTXO{utxo1})
			if err != nil {
				t.Fatal(err)
			}

			importTx2, err := atomic.NewImportTx(vm.ctx, vm.currentRules(), vm.clock.Unix(), vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(big.NewInt(3), initialBaseFee), kc, []*lux.UTXO{utxo2})
			if err != nil {
				t.Fatal(err)
			}

			reissuanceTx1, err := atomic.NewImportTx(vm.ctx, vm.currentRules(), vm.clock.Unix(), vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(big.NewInt(2), initialBaseFee), kc, []*lux.UTXO{utxo1, utxo2})
			if err != nil {
				t.Fatal(err)
			}
			...
			reissuanceTx2, err := atomic.NewImportTx(vm.ctx, vm.currentRules(), vm.clock.Unix(), vm.ctx.XChainID, testEthAddrs[0], new(big.Int).Mul(big.NewInt(4), initialBaseFee), kc, []*lux.UTXO{utxo1, utxo2})
			if err != nil {
				t.Fatal(err)
			}
...
func TestBuildBlockDoesNotExceedAtomicGasLimit(t *testing.T) {
	...
	for i := 0; i < mempoolTxs; i++ {
		utxo, err := addUTXO(sharedMemory, vm.ctx, txID, uint32(i), vm.ctx.LUXAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)

		importTx, err := atomic.NewImportTx(vm.ctx, vm.currentRules(), vm.clock.Unix(), vm.ctx.XChainID, testEthAddrs[0], initialBaseFee, kc, []*lux.UTXO{utxo})
		if err != nil {
			t.Fatal(err)
		}
		if err := vm.mempool.AddLocalTx(importTx); err != nil {
...
func TestExtraStateChangeAtomicGasLimitExceeded(t *testing.T) {
	...
	// in ApricotPhase5.
	for i := 0; i < 100; i++ {
		_, err := addUTXO(sharedMemory1, vm1.ctx, txID, uint32(i), vm1.ctx.LUXAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)

		_, err = addUTXO(sharedMemory2, vm2.ctx, txID, uint32(i), vm2.ctx.LUXAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)
	}
