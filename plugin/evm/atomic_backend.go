...
func NewAtomicBackend(
	db *versiondb.Database, sharedMemory luxatomic.SharedMemory,
...
func (a *atomicBackend) ApplyToSharedMemory(lastAcceptedBlock uint64) error {
	...
	batchOps := make(map[ids.ID]*luxatomic.Requests)
	for it.Next() {
...
func mergeAtomicOps(txs []*atomic.Tx) (map[ids.ID]*luxatomic.Requests, error) {
	...
	output := make(map[ids.ID]*luxatomic.Requests)
	for _, tx := range txs {
...
func mergeAtomicOpsToMap(output map[ids.ID]*luxatomic.Requests, chainID ids.ID, requests *luxatomic.Requests) {
