...
import (
	...
	warpcontract "github.com/ethereum/go-ethereum/precompile/contracts/warp"
...
func (vm *VM) Initialize(
	...
	gethLogger, err := InitLogger(vm.chainAlias, vm.config.LogLevel, vm.config.LogJSONFormat, writer)
	if err != nil {
		return fmt.Errorf("failed to initialize logger due to: %w ", err)
	}
	vm.logger = gethLogger

	log.Info("Initializing Geth VM", "Version", Version, "Config", vm.config)
	...
	if vm.ctx.NetworkID == nodeConstants.MainnetID {
...
func (vm *VM) preBatchOnFinalizeAndAssemble(header *types.Header, state *state.StateDB, txs []*types.Transaction) ([]byte, *big.Int, *big.Int, error) {
	for {
		...
		if rules.IsApricotPhase4 {
			contribution, gasUsed, err = tx.BlockFeeContribution(rules.IsApricotPhase5, vm.ctx.LUXAssetID, header.BaseFee)
			if err != nil {
...
func (vm *VM) postBatchOnFinalizeAndAssemble(
	...
	for {
		...
		// ApricotPhase5.
		txContribution, txGasUsed, err = tx.BlockFeeContribution(true, vm.ctx.LUXAssetID, header.BaseFee)
		if err != nil {
			return nil, nil, nil, err
		}
...
func (vm *VM) onExtraStateChange(block *types.Block, parent *types.Header, state *state.StateDB) (*big.Int, *big.Int, error) {
	...
	for _, tx := range txs {
		...
		if rules.IsApricotPhase4 {
			contribution, gasUsed, err := tx.BlockFeeContribution(rules.IsApricotPhase5, vm.ctx.LUXAssetID, block.BaseFee())
			if err != nil {
				return nil, nil, err
			}
...
func (vm *VM) CreateHandlers(context.Context) (map[string]http.Handler, error) {
	...
	if vm.config.AdminAPIEnabled {
		adminAPI, err := newHandler("admin", NewAdminService(vm, os.ExpandEnv(fmt.Sprintf("%s_geth_performance_%s", vm.config.AdminAPIDir, vm.chainAlias))))
		if err != nil {
			return nil, fmt.Errorf("failed to register service for admin API due to %w", err)
		}
		apis[adminEndpoint] = adminAPI
		enabledAPIs = append(enabledAPIs, "geth-admin")
	}
