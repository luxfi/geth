...
func mustAtomicOps(tx *atomic.Tx) map[ids.ID]*luxatomic.Requests {
	id, reqs, err := tx.AtomicOps()
	if err != nil {
		panic(err)
	}
	return map[ids.ID]*luxatomic.Requests{id: reqs}
}
...
func indexAtomicTxs(tr AtomicTrie, height uint64, atomicOps map[ids.ID]*luxatomic.Requests) error {
...
func TestIndexingNilShouldNotImpactTrie(t *testing.T) {
	// operations to index
	ops := make([]map[ids.ID]*luxatomic.Requests, 0)
...
type sharedMemories struct {
	thisChain   luxatomic.SharedMemory
	peerChain   luxatomic.SharedMemory
	thisChainID ids.ID
	peerChainID ids.ID
}

func (s *sharedMemories) addItemsToBeRemovedToPeerChain(ops map[ids.ID]*luxatomic.Requests) error {
	for _, reqs := range ops {
		puts := make(map[ids.ID]*luxatomic.Requests)
		puts[s.thisChainID] = &luxatomic.Requests{}
		for _, key := range reqs.RemoveRequests {
			val := []byte{0x1}
			puts[s.thisChainID].PutRequests = append(puts[s.thisChainID].PutRequests, &luxatomic.Element{Key: key, Value: val})
		}
...
func (s *sharedMemories) assertOpsApplied(t *testing.T, ops map[ids.ID]*luxatomic.Requests) {
	t.Helper()
	for _, reqs := range ops {
...
func (s *sharedMemories) assertOpsNotApplied(t *testing.T, ops map[ids.ID]*luxatomic.Requests) {
	t.Helper()
	for _, reqs := range ops {
...
func newSharedMemories(atomicMemory *luxatomic.Memory, thisChainID, peerChainID ids.ID) *sharedMemories {
