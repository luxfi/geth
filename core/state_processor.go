...
// SetVMConfig sets the VM configuration for the processor.
func (p *StateProcessor) SetVMConfig(vmConfig *vm.Config) {
	p.vmConfig = vmConfig
}
...
func MakeReceipt(evm *vm.EVM, result *ExecutionResult, statedb *state.StateDB, blockNumber *big.Int, blockHash common.Hash, blockTime uint64, tx *types.Transaction, usedGas uint64, root []byte) *types.Receipt {
	...
	if tx.To() == nil {
		// Convert geth common.Address to crypto.Address for CreateAddress function
		var cryptoAddr [20]byte
		copy(cryptoAddr[:], evm.TxContext.Origin[:])
		createdAddr := crypto.CreateAddress(cryptoAddr, tx.Nonce())
		// Convert crypto.Address back to geth common.Address
