...
func TestDeleteStorage(t *testing.T) {
	...
	_, _, fastNodes, err := fastState.deleteStorage(addr, common.Keccak256Hash(addr[:]), storageRoot)
	if err != nil {
		t.Fatal(err)
	}

	_, _, slowNodes, err := slowState.deleteStorage(addr, common.Keccak256Hash(addr[:]), storageRoot)
	if err != nil {
		t.Fatal(err)
	}
