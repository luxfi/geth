...
func newTriePrefetcher(db Database, root common.Hash, namespace string, concurrency ...int) *triePrefetcher {
...
func (p *triePrefetcher) close() {
	for _, fetcher := range p.fetchers {
		...
		if true { // metrics.Enabled removed in go-ethereum v1.16.1
...
func (p *triePrefetcher) prefetch(owner common.Hash, root common.Hash, addr common.Address, keys [][]byte) {
	...
	if fetcher == nil {
		fetcher = newSubfetcher(p.db, p.root, owner, root, addr, p.concurrency)
...
func newSubfetcher(db Database, state common.Hash, owner common.Hash, root common.Hash, addr common.Address, concurrency int) *subfetcher {
	...
	if concurrency > 1 {
		sf.workers = make(chan struct{}, concurrency)
	}
	go sf.loop()
	return sf
}
...
func (sf *subfetcher) loop() {
	...
	defer func() {
		sf.wg.Wait() // Wait for all workers to finish
		close(sf.term)
	}()
						...
						if sf.concurrency > 1 {
							...
							go func(task []byte) {
								...
								if len(task) == common.AddressLength {
									...
									if _, err := trie.GetStorage(sf.addr, task); err != nil {
										log.Error("storage prefetching failed", "address", sf.addr, "key", task, "err", err)
									}
								}
							}(task)
						} else {
							...
							if len(task) == common.AddressLength {
								if _, err := sf.trie.GetAccount(common.BytesToAddress(task)); err != nil {
									log.Error("account prefetching failed", "address", common.BytesToAddress(task), "err", err)
								}
							} else {
								if _, err := sf.trie.GetStorage(sf.addr, task); err != nil {
									log.Error("storage prefetching failed", "address", sf.addr, "key", task, "err", err)
								}
							}
						}
