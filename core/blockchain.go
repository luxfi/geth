...
func (bc *BlockChain) insertSideChain(block *types.Block, it *insertIterator, makeWitness bool) (*stateless.Witness, int, error) {
	...
	for parent != nil && !bc.HasState(parent.Root) {
		...
		// If we reach genesis without state, we cannot import - state is required for proper verification
		if parent.Number.Uint64() == 0 {
			return nil, it.index, errors.New("genesis state missing: cannot import blocks without proper state re-execution. Initialize chain with genesis state first")
		}
