...
func fillPool(t testing.TB, pool *LegacyPool) {
	...
	for i := 0; i < 384; i++ {
		...
		pool.currentState.AddBalance(common.Address(crypto.PubkeyToAddress(key.PublicKey)), uint256.NewInt(10000000000), tracing.BalanceChangeUnspecified)
		// Add executable ones
...
func TestTransactionFutureAttack(t *testing.T) {
	...
	{
		key, _ := crypto.GenerateKey()
		pool.currentState.AddBalance(common.Address(crypto.PubkeyToAddress(key.PublicKey)), uint256.NewInt(100000000000), tracing.BalanceChangeUnspecified)
