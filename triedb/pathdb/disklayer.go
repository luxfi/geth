...
func (dl *diskLayer) node(owner common.Hash, path []byte, depth int) ([]byte, common.Hash, *nodeLoc, error) {
	...
	if dl.nodes != nil {
		if blob := dl.nodes.Get(nil, key); len(blob) > 0 {
			...
			return blob, common.Keccak256Hash(blob), &nodeLoc{loc: locCleanCache, depth: depth}, nil
		}
	...
	return blob, common.Keccak256Hash(blob), &nodeLoc{loc: locDiskLayer, depth: depth}, nil
}
